...
다중 스레드 환경에서 안전하게 작동할 수 있도록 `ReentrantLock`을 활용하여 **유저별로 락을 관리**하며, 포인트 사용 시 발생할 수 있는 **포인트 부족** 문제를 처리하기 위해 **큐**를 사용하여 실패한 요청을 관리하고 재시도합니다.
아래에서는 동시성 문제 해결을 위한 코드 설계와 테스트 방법에 대해 설명합니다.
